    ...
    def observe(self, x, t, y):
        ...
        for i in range(0, x.size()[0]):
            ...
            if self.age > 1:
                ...
                if self.age > 2000:
                    xi = Variable(torch.from_numpy(np.array(xi))).float().view(1, -1)
                    yi = Variable(torch.from_numpy(np.array(yi))).long().view(1)
                    ...
                    prediction = self.forward(xi, 0)
                    loss = self.bce(prediction, yi)
                    loss.backward()
                    self.net.opt(1)
                else:
                    weights_before = deepcopy(self.net.state_dict())
