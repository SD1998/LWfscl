...
class Net(nn.Module):
    def __init__(self,args):
        ...
        if self.is_cub:
            ...
            self.nc_per_task = 10
            self.n_class = 200
            self.encoder = resnet18(True, self.n_class)  # pretrained=True follow TOPIC, models for cub is imagenet pre-trained. https://github.com/xyutao/fscil/issues/11#issuecomment-687548790
        ...
        self.weights_before1 = deepcopy(self.encoder.state_dict())
        self.weights_before = deepcopy(self.classifer.state_dict())
    ...
    def observe(self, x, t, y,flags,flagm=False):
        ...
        if t==0:
            ...
            if flagm:
                ...
                self.encoder.load_state_dict(
                    {name: self.weights_before1[name] + ((weights_after1[name] - self.weights_before1[name]) * self.args.gamma) for
                     name in self.weights_before1})
                self.classifer.meta_learn(self.weights_before, weights_after)
                self.weights_before = deepcopy(self.classifer.state_dict())
                self.weights_before1 = deepcopy(self.encoder.state_dict())
        else:
            for i in range(0, x.size()[0]):
                ...
                if self.age > 1:
                    ...
                    # Reptile meta-update
                    if self.flag1:
                        self.encoder.load_state_dict({name: weights_before1[name] + ((weights_after1[name] - weights_before1[name])* self.args.new_gamma) for name in weights_before1})
