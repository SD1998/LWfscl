...
from metrics.metrics1 import confusion_matrix
#
...
class Trainer():
    ...
    def get_traindataloader(self,session):
        if session == 0:
            trainset, trainloader = self.get_base_traindataloader_meta()
        else:
            trainset, trainloader= self.get_new_traindataloader(session)
        return trainset, trainloader
    def get_testdataloader(self,session):
        if session == 0:
            testloader = self.get_base_testdataloader_meta()
        else:
            testloader = self.get_new_testdataloader(session)
        return testloader
    def get_base_traindataloader_meta(self):
        ...
        return trainset, trainloader
    def get_base_testdataloader_meta(self):
        txt_path = "data/index_list/" + self.args.dataset + "/session_" + str(0 + 1) + '.txt'
        class_index = np.arange(self.args.base_class)
        if self.args.dataset == 'cifar100':
        ...
        return testloader
    def get_new_traindataloader(self, session):
        ...
        return trainset, trainloader
    def get_new_testdataloader(self, session):
    ...
    #
        for session in range(self.args.sessions):
            testloader =self.get_testdataloader(session)
        ...
        for session in range(self.args.sessions):
            ...
            if self.args.pretrain and session==0:
                ...
                result_m,result_list = self.eval_tasks(session)
                print(result_m,result_list)
            else:
                train_set, trainloader = self.get_traindataloader(session)
                if session==0:
                ...
                for epoch in range(epochs):
                    ...
                    for (i, data) in enumerate(trainloader):
                        ...
                        self.model.train()
                        if i!=0 and i%2==0:
                           self.model.observe(Variable(v_x), t, Variable(v_y),flag1,True)
                        else:
                           self.model.observe(Variable(v_x), t, Variable(v_y), flag1)
                        torch.cuda.empty_cache()
                    result_tem, result_temlist = self.eval_tasks(session)
                    print(epoch,result_tem,result_temlist)
                    if result_tem > result_m:
                        result_m = result_tem
                        result_list =result_temlist
                ...
                result_t.append(t)
                result_l.append(result_list)

        time_end = time.time()
        time_spent = time_end - time_start

        return torch.Tensor(result_t), torch.Tensor(result_l),torch.Tensor(result_a), time_spent
...
if __name__ == "__main__":
    ...
    result_t, result_list, result_a, spent_time = train_tool.life_experience()
    ...
    stats = confusion_matrix(result_t, result_list, result_a,args,spent_time,fname + '.txt')
    one_liner = str(vars(args)) + ' # '
    one_liner += ' '.join(["%.3f" % stat for stat in stats])
    print(fname + ': ' + one_liner + ' # ' + str(spent_time))

    # save all results in binary file
    torch.save((result_t, result_list, model.state_dict(),
