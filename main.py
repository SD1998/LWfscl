...
def eval_tasks(model, tasks, args):
    model.eval()
    result = []
    is_tiered = (args.data_file == 'tieredimagenet.pt')
    is_office = (args.data_file == 'office.pt')
    is_fru = (args.data_file=='fru.pt')
    for i, task in enumerate(tasks):
        ...
        with torch.no_grad():  # torch 0.4+
            for b_from in range(0, x.size(0), eval_bs):
                ...
                if is_tiered:
                    offset1, offset2 = compute_offsets(t, 4, True)
                    _, pb = torch.max(model(xb, t)[:, offset1: offset2].data.cpu(), 1, keepdim=False)
                elif is_office:
                    offset1, offset2 = compute_offsets(t, 10, True)
                    _, pb = torch.max(model(xb, t)[:, offset1: offset2].data.cpu(), 1, keepdim=False)
                elif is_fru:
                    offset1, offset2 = compute_offsets(t, 5, True)
                    _, pb = torch.max(model(xb, t)[:, offset1: offset2].data.cpu(), 1, keepdim=False)
                else:
...
if __name__ == "__main__":
    ...
    aaa=fname + ': ' + one_liner + ' # ' + str(spent_time)
    print(aaa)
    if fname+'.txt' is not None:
        f=open(fname+'.txt','a')
        print(aaa,file=f)
        f.close()
